...
import { FaBoxOpen, FaCheckCircle, FaTruck, FaMapMarkerAlt, FaTimesCircle, FaChevronRight, FaUndoAlt } from "react-icons/fa";
...
const OrderHistoryPage = () => {
  ...
  const handleConfirmDelivery = async (orderId) => {
    try {
      ...
      setOrders((prev) => prev.map((o) => o._id === orderId ? { ...o, status: "delivered", paymentStatus: "paid" } : o));
  ...
  const handleReturnRequest = async (orderId) => {
    ...
    try {
      ...
      setOrders((prev) => prev.map((o) => o._id === orderId ? { ...o, status: "return-request" } : o));
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 md:px-8 py-5 border-b border-slate-100 bg-slate-50/50 gap-4">
                  <div>
                    <div className="flex items-center gap-3 mb-1">
                      ...
                      <span className={`px-3 py-1 rounded-full text-xs font-bold border flex items-center gap-1.5 ${STATUS_COLORS[order.status] || "bg-slate-50 text-slate-600 border-slate-200"}`}>
                ...
                <div className="p-6 md:p-8">
                  ...
                  {(order.status !== "canceled" && order.status !== "return-request" && order.status !== "rejected") && (
                    <div className="relative mb-8 pt-4">
                      ...
                      <div className="absolute top-8 left-0 h-1 bg-primary rounded-full transition-all duration-500" style={{ width: `${Math.max(0, getTrackingProgress(order.status) / 3 * 100)}%` }}></div>
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-6 border-t border-slate-100">
                    ...
                    <div className="flex flex-wrap gap-3 w-full md:w-auto">
                      {order.status === "shipped" && (
                        isLoggedIn ? (
                          <button onClick={() => handleConfirmDelivery(order._id)} className="flex-1 md:flex-none px-6 py-3 bg-green-500 text-white font-bold rounded-xl hover:bg-green-600 transition-colors shadow-lg shadow-green-500/20">Đã Nhận Hàng</button>
                        ) : (<button onClick={() => navigate("/login")} className="flex-1 md:flex-none px-6 py-3 bg-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-300 transition-colors">Đăng nhập xác nhận</button>)
                      )}

                      {order.status === "delivered" && (
                        isLoggedIn ? (
                          <button onClick={() => handleReturnRequest(order._id)} className="flex-1 md:flex-none px-6 py-3 bg-orange-100 text-orange-600 font-bold rounded-xl hover:bg-orange-200 transition-colors">Yêu Cầu Hoàn Hàng</button>
                        ) : (<button onClick={() => navigate("/login")} className="flex-1 md:flex-none px-6 py-3 bg-slate-200 text-slate-700 font-bold rounded-xl hover:bg-slate-300 transition-colors">Đăng nhập hoàn hàng</button>)
                      )}

                      {(order.status === "pending" || order.status === "confirmed") && (
                        <button onClick={() => handleCancelOrder(order._id)} className="flex-1 md:flex-none px-6 py-3 border-2 border-slate-200 text-slate-500 font-bold rounded-xl hover:border-red-200 hover:text-red-500 hover:bg-red-50 transition-colors">Hủy Đơn Hàng</button>
                      )}
                    </div>
                  </div>
                </div>
